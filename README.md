...
Web push notifications for the admin app require a public VAPID key provided at build/runtime.

1. Get the VAPID public key from Firebase Console:
  Project settings > Cloud Messaging > Web push certificates.
2. Run locally with:

  ```bash
  flutter run -d chrome --dart-define=FIREBASE_WEB_PUSH_VAPID_KEY=YOUR_VAPID_PUBLIC_KEY
  ```

3. Build for production with:

  ```bash
  flutter build web --dart-define=FIREBASE_WEB_PUSH_VAPID_KEY=YOUR_VAPID_PUBLIC_KEY
  ```

The VAPID key is public and can be exposed client-side.
